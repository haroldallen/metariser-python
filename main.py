...
# User Config
folder = input('Enter the folder directory: ')
...
for file in f:
    ...
    # Open image
    old_im = Image.open(folder + "/" + file)
    meta_im = old_im.copy()
    
    # Get border and size units (border is double the actual border size)
    old_size = old_im.size
    border = min(old_im.size[0] // 20, old_im.size[1] // 20)
    ...
    if path.exists(folder + output_folder) == False:
        os.makedirs(folder + output_folder)
    new_im.save(folder + output_folder + file)
    ...
    if config_meta:
        ...
        parser = createParser(file, folder + "/" + file)
        if not parser:
            ...
            for index, line in enumerate(text):
                ...
                x = round(border*2+size+y_unit/3)
                ...
                if "Camera focal" in line:
                    ...
                    y = border*1.5+(main_size+y_unit/3)
                    ...
                    y = border*1.5+(main_size+y_unit/3)*2
                    ...
                    y = border*1.5+(main_size+y_unit/3)*3
                    ...
                    y = border*1.5+(main_size+y_unit/3)*4+y_unit/6
                    ...
                    y = border*1.5+(main_size+y_unit/3)*4+size+y_unit/3
                else:
                    continue
            ...
            draw.text((border*2, new_size[1]-border*2-font_watermark.size), app_url, fill=text_color, font=font_watermark)
            
            info_im.save(folder + output_folder + file[:-3] + 'meta.jpg')
